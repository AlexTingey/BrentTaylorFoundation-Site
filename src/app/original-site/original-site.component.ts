...
@Component({
  ...
  templateUrl: './original-site.component.html',
  styleUrls: ['./original-site.component.scss']
})
export class OriginalSiteComponent implements OnInit {
  ...
  displayMoreText(){
    console.log("linked");
    var OurMission = document.getElementById("read-more");
  ...
  showButton(button : string){
    var Donation = document.getElementById(button);
    var Button = document.querySelector('.filter-active');

    Button.classList.remove("filter-active");
    var NewButton = document.querySelector("." + button);
    console.log(Button);
    NewButton.classList.add("filter-active");
    var oldID = Button.classList[0];
    var oldDonation = document.getElementById(oldID);
